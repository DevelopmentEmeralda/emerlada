...
<?php
    ...
    if(sw_count($file_rep))
    {
        ...
        // {
        //     echo '<img width="100" height="100" src="'.base_url('files/'.$file_rep[0]->filename).'" alt="'.lang_check('floorplan').'">';
        // }
        ...
        foreach($file_rep as $file_r)
        {
            ...
            if(file_exists(FCPATH.'/files/thumbnail/'.$file_r->filename))
            {
                ...
                '        <img width="100%" height="100%" src="'.base_url('files/thumbnail/'.$file_r->filename).'" data-src="'.base_url('files/'.$file_r->filename).'" alt="'.$file_r->filename.'" class="" />'.
                '    </a>'.
                '    </div>'.
                '</div>';
            }
            else if(file_exists(FCPATH.'/templates/'.$settings_template.'/assets/img/icons/filetype/'.get_file_extension($file_r->filename).'.png'))
            {
                ...
                '    <a target="_blank" href="'.base_url('files/'.$file_r->filename).'" title="'.$file_r->filename.'" download="'.base_url('files/'.$file_r->filename).'" class="preview show-icon direct-download">'.
                '        <img width="100%" height="100%" src="assets/img/icons/filetype/'.get_file_extension($file_r->filename).'.png" data-src="'.base_url('files/'.$file_r->filename).'" alt="'.$file_r->filename.'" class="" />'.
                '    </a>'.
                '    </div>'.
                '</div>';
            }
